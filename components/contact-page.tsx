...
function isValidProjectType(value: string): boolean {
  return projectTypes.some((type) => type.value === value);
}

export default function ContactPage({
  initialWebsiteType = "",
}: ContactPageProps) {
  const fallbackWebsiteType = useMemo(
    () => (isValidProjectType(initialWebsiteType) ? initialWebsiteType : ""),
    [initialWebsiteType]
  );
  const [preselectedWebsiteType, setPreselectedWebsiteType] = useState(fallbackWebsiteType);
  ...
  const [form, setForm] = useState({
    ...
    phone: "",
    company: "",
    websiteType: fallbackWebsiteType,
    budget: 1000,
    message: "",
  });
  ...
  useEffect(() => {
    const nextWebsiteType =
      typeof window === "undefined"
        ? fallbackWebsiteType
        : (() => {
            const queryValue = new URLSearchParams(window.location.search).get("websiteType");
            return queryValue && isValidProjectType(queryValue)
              ? queryValue
              : fallbackWebsiteType;
          })();

    setPreselectedWebsiteType((prev) =>
      prev === nextWebsiteType ? prev : nextWebsiteType
    );
  }, [fallbackWebsiteType]);
  ...
  const budgetText = useMemo(
    () => budgetLabels[form.budget] ?? `${form.budget} €`,
    [form.budget]
  );
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!agbAccepted) {
      setSubmitError("Bitte akzeptieren Sie die AGB, um fortzufahren.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSubmitError(data.error || "Die Anfrage konnte nicht gesendet werden.");
      }
    } catch (error) {
      console.error("Submit error:", error);
      setSubmitError("Die Anfrage konnte gerade nicht gesendet werden.");
    } finally {
    ...
    <div className="relative min-h-screen overflow-hidden text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 pb-24 pt-8 sm:px-6 lg:px-8 lg:pt-16">
        <section className="mx-auto max-w-3xl text-center">
          ...
          >
            <span className="text-[#2997ff]">Erzählen</span> Sie uns kurz, was Sie{" "}
            <span className="text-[#2997ff]">brauchen</span>.
          </motion.h1>
          ...
          >
            Je klarer Ihre Angaben, desto passender können wir Ihr Konzept und Ihre
            Website vorbereiten.
          </motion.p>
          ...
          >
            <div className="mb-8">
              ...
              <p className="mt-3 text-sm leading-6 text-white/52">
                Wenige klare Angaben genügen, damit Ihre Anfrage sauber eingeschätzt
                werden kann.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Field label="Budgetvorstellung">
                <div className="rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-5 backdrop-blur-xl">
                  <div className="mb-5 flex items-center justify-between gap-4 text-sm text-white/70">
                    ...
                    <span className="rounded-full border border-[#2997ff]/30 bg-[#2997ff]/10 px-4 py-1.5 text-sm font-medium text-[#2997ff]">
                      {budgetText}
                    </span>
                  </div>
              ...
              <div className="flex items-start gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4">
                <input
                  ...
                  className="mt-1 h-5 w-5 cursor-pointer rounded border-white/20 bg-white/5 text-[#2997ff] focus:ring-[#2997ff]"
                />
                <label htmlFor="agb" className="cursor-pointer text-sm text-white/60">
                  Ich habe die{" "}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-sm text-emerald-100/75">
                          Vielen Dank. Die Anfrage ist eingegangen und wird zeitnah
                          bearbeitet.
                        </div>
                      </div>
                    </div>
