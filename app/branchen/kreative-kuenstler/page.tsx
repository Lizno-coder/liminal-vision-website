...
const features = [
  ...
  { icon: Mail, title: "Buchungsanfragen", desc: "Direkte Kontaktaufnahme" },
  { icon: Palette, title: "Preisliste", desc: "Transparente Kosten" },
  { icon: Brush, title: "Kunst-Galerie", desc: "Werke verkaufen" },
];

const benefits = [
  "5x mehr Aufträge durch professionelles Portfolio",
  "Weltweite Sichtbarkeit für Ihre Kunst",
  "Direkte Buchungen ohne Vermittler",
  "Professioneller Auftritt gegenüber Galerien",
  "Einfache Verwaltung von Anfragen und Terminen",
];

const stats = [
  { value: "5x", label: "mehr Aufträge" },
  { value: "Global", label: "Sichtbarkeit" },
...
export default function KreativePage() {
  return (
    <div className="min-h-screen bg-[#08090d] text-white overflow-x-hidden">
      <div className="fixed inset-0 bg-[linear-gradient(rgba(168,85,247,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(168,85,247,0.03)_1px,transparent_1px)] bg-[size:80px_80px] pointer-events-none" />
        ...
        <div className="mx-auto max-w-6xl">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#a855f7]/10 border border-[#a855f7]/30 text-[#a855f7] text-sm font-medium mb-6">
                <Brush className="w-4 h-4" />Kreative & Künstler
              </div>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-6">
                Ihr Portfolio verdient{" "}
              ...
              <div className="relative rounded-3xl overflow-hidden border border-white/10 bg-gradient-to-br from-[#a855f7]/10 to-transparent p-2">
                <div className="aspect-[4/3] rounded-2xl bg-gradient-to-br from-[#1a1a2e] to-[#0a0a0f] flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-white/40 text-sm">Künstler Portfolio</p>
                  </div>
                </div>
              </div>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="absolute -bottom-6 -left-6 bg-[#0d0d0f] border border-white/10 rounded-2xl p-4 shadow-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-white/50">mehr Aufträge</div>
                  </div>
                </div>
              </motion.div>
            </motion.div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {stats.map((stat, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="text-center">
                <div className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-[#a855f7] to-[#c084fc] bg-clip-text text-transparent mb-2">{stat.value}</div>
      ...
      <section className="relative px-4 py-24">
        <div className="mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            ...
            <p className="text-white/60 max-w-xl mx-auto">Perfekt für Fotografen, Designer, Musiker & Künstler</p>
          </motion.div>
            ...
            {features.map((feature, index) => {
              ...
              return (
                <motion.div key={index} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="group">
                  <div className="h-full rounded-2xl border border-white/10 bg-white/[0.02] p-6 transition-all duration-300 hover:border-[#a855f7]/30 hover:bg-white/[0.04]">
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <h2 className="text-3xl md:text-4xl font-bold mb-6">Warum Kreative eine Website brauchen</h2>
              ...
              <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.05] to-transparent p-8">
                <div className="flex items-center gap-4 mb-6">
                  <div className="flex -space-x-2">
                    {[1,2,3].map((i) => (
                      <div key={i} className="w-10 h-10 rounded-full bg-gradient-to-br from-[#a855f7] to-[#c084fc] flex items-center justify-center text-white font-bold text-sm border-2 border-[#0d0d0f]">
                ...
                <blockquote className="text-white/80 italic mb-4">
                  "Mein Portfolio hat meine Karriere verändert. Ich bekomme Aufträge aus der ganzen Welt."
                </blockquote>
                <div className="text-sm">
                  <span className="text-white font-semibold">Julia K.</span>
                  <span className="text-white/50"> — Fotografin, Berlin</span>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <section className="relative px-4 py-24">
        <div className="mx-auto max-w-4xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#a855f7]/10 via-transparent to-[#c084fc]/10 p-8 md:p-12">
