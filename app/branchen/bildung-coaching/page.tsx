...
const features = [
  ...
  { icon: BookOpen, title: "Buchungssystem", desc: "Online anmelden & bezahlen" },
  { icon: Trophy, title: "Erfolgsgeschichten", desc: "Referenzen & Feedback" },
  { icon: GraduationCap, title: "Lehrmaterial", desc: "Downloads & Ressourcen" },
];

const benefits = [
  "200% mehr Buchungen durch Online-Anmeldung",
  "Reduzierter Verwaltungsaufwand",
  "Bessere Schüler/Berufskunden-Gewinnung",
  "Professioneller Auftritt gegenüber Konkurrenz",
  "Einfache Verwaltung von Kursen und Terminen",
];

const stats = [
  { value: "+200%", label: "mehr Buchungen" },
  { value: "85%", label: "suchen online" },
  { value: "24/7", label: "Anmeldung möglich" },
];
    ...
    <div className="min-h-screen bg-[#08090d] text-white overflow-x-hidden">
        ...
        <div className="mx-auto max-w-6xl">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#f97316]/10 border border-[#f97316]/30 text-[#f97316] text-sm font-medium mb-6">
                <GraduationCap className="w-4 h-4" />Bildung & Coaching
              </div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative">
              <div className="relative rounded-3xl overflow-hidden border border-white/10 bg-gradient-to-br from-[#f97316]/10 to-transparent p-2">
                <div className="aspect-[4/3] rounded-2xl bg-gradient-to-br from-[#1a1a2e] to-[#0a0a0f] flex items-center justify-center">
                  <div className="text-center">
                    <div className="inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-[#f97316]/20 mb-4">
                      <BookOpen className="w-12 h-12 text-[#f97316]" />
                    </div>
                    <p className="text-white/40 text-sm">Bildung Website</p>
                  </div>
                </div>
              </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {stats.map((stat, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="text-center">
                <div className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-[#f97316] to-[#fb923c] bg-clip-text text-transparent mb-2">{stat.value}</div>
      ...
      <section className="relative px-4 py-24">
        <div className="mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            ...
            <p className="text-white/60 max-w-xl mx-auto">Perfekt für Nachhilfe, Coaching & Weiterbildung</p>
          </motion.div>
            ...
            {features.map((feature, index) => {
              ...
              return (
                <motion.div key={index} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="group">
                  <div className="h-full rounded-2xl border border-white/10 bg-white/[0.02] p-6 transition-all duration-300 hover:border-[#f97316]/30 hover:bg-white/[0.04]">
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <h2 className="text-3xl md:text-4xl font-bold mb-6">Warum Bildungsanbieter eine Website brauchen</h2>
              ...
              <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.05] to-transparent p-8">
                <div className="flex items-center gap-4 mb-6">
                  <div className="flex -space-x-2">
                    {[1,2,3].map((i) => (
                      <div key={i} className="w-10 h-10 rounded-full bg-gradient-to-br from-[#f97316] to-[#fb923c] flex items-center justify-center text-white font-bold text-sm border-2 border-[#0d0d0f]">
                ...
                <blockquote className="text-white/80 italic mb-4">
                  "Unsere Kurse sind seit der neuen Website immer ausgebucht. Die Online-Anmeldung funktioniert perfekt."
                </blockquote>
                <div className="text-sm">
                  <span className="text-white font-semibold">Anna L.</span>
                  <span className="text-white/50"> — Lernakademie, München</span>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <section className="relative px-4 py-24">
        <div className="mx-auto max-w-4xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#f97316]/10 via-transparent to-[#fb923c]/10 p-8 md:p-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">Bereit für mehr Schüler?</h2>
